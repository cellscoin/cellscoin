...
#include "zcellscoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCELLSCOIN ought to be enough for anybody." - Bill Gates, 2017
    ui->zCELLSCOINpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCELLSCOINSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCELLSCOIN->setEnabled(false);
        ui->pushButtonMintzCELLSCOIN->setToolTip(tr("zCELLSCOIN is currently disabled due to maintenance."));

        ui->pushButtonSpendzCELLSCOIN->setEnabled(false);
        ui->pushButtonSpendzCELLSCOIN->setToolTip(tr("zCELLSCOIN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCELLSCOINpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCELLSCOIN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCELLSCOIN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCELLSCOIN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCELLSCOIN in ") + 
...
void PrivacyDialog::on_pushButtonSpendzCELLSCOIN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCELLSCOIN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCELLSCOIN
        sendzCELLSCOIN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCELLSCOIN
    sendzCELLSCOIN();
}

void PrivacyDialog::on_pushButtonZCellscoinControl_clicked()
{
    ZCellscoinControlDialog* zCellscoinControl = new ZCellscoinControlDialog(this);
    zCellscoinControl->setModel(walletModel);
    zCellscoinControl->exec();
}

void PrivacyDialog::setZCellscoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCellscoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCELLSCOIN()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cellscoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCELLSCOINpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCELLSCOIN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CELLSCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCELLSCOIN</b>";
    ...
    // use mints from zCellscoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCellscoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCellscoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcellscoin selector in case it was used
    ZCellscoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCellscoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Cellscoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCellscoin Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zCELLSCOINpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCELLSCOIN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCELLSCOIN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCELLSCOIN "));
    ui->labelzCELLSCOINAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCELLSCOINSyncStatus->setVisible(fShow);
}
