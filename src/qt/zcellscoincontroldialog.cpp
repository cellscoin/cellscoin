...
#include "ui_zcellscoincontroldialog.h"
...
std::list<std::string> ZCellscoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCellscoinControlDialog::listMints;

ZCellscoinControlDialog::ZCellscoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCellscoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCellscoinControlDialog::~ZCellscoinControlDialog()
{
    delete ui;
}

void ZCellscoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCellscoinControlDialog::updateList()
{
...
void ZCellscoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCellscoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCellscoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCellscoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCellscoinControlDialog::GetSelectedMints()
{
...
void ZCellscoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
